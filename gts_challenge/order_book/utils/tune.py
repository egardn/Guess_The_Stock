...
DEFAULT_PREPROCESSED_DIR = PROJECT_ROOT / "data" / "preprocessed_data" # Updated preprocessed path
DEFAULT_CHECKPOINT_DIR_BASE = PROJECT_ROOT / "models" # <--- Changed default checkpoint base
...
def objective(trial: optuna.trial.Trial, model_type: str,
    ...
    # Checkpoints for this specific trial will go into a subdirectory named after the model type and trial number
    trial_checkpoint_dir = checkpoint_dir_base / f"{model_type}_trial_{trial.number}" # <--- Subdir structure within base
    try:
    ...
    try:
        ...
        # Determine step based on model type
        report_step = -1
        if model_type == 'gru':
            report_step = params.get('epochs', -1)
        elif model_type == 'gb':
            report_step = params['model_params'].get('n_estimators', -1)
        trial.report(final_metric, step=report_step)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--checkpoint_dir_base', type=str, default=str(DEFAULT_CHECKPOINT_DIR_BASE), # <--- Uses updated default
    ...
    checkpoint_dir_base_path = Path(args.checkpoint_dir_base).resolve() # Use the argument value
    ...
    try:
        ...
        checkpoint_dir_base_path.mkdir(parents=True, exist_ok=True) # Ensure the base 'models' dir exists
    ...
    logger.info(f"Using Checkpoint Base Dir: {checkpoint_dir_base_path}") # Log the actual base path being used
    ...
    objective_func = lambda trial: objective(
        ...
        checkpoint_dir_base=checkpoint_dir_base_path # Pass the resolved base path
    )
    ...
    try:
        ...
        if not completed_trials:
            ...
            # Log the location of the best trial's checkpoints using the correct base path
            best_trial_dir = checkpoint_dir_base_path / f"{args.model_type}_trial_{best_trial.number}"
            logger.info(f"  Checkpoints saved in: {best_trial_dir}") # <--- Uses updated base path
